...
export async function getUserInfo(token: string, email?: string): Promise<AuthResponse> {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
    ...
    console.log('Obteniendo info del usuario con token:', token.substring(0, 20) + '...');
    ...
    if (!response.ok) {
      console.log('Error al obtener usuario:', response.status, response.statusText);
      ...
      return {
        success: false,
        error: errorData.message || errorData.error || 'Error al obtener información del usuario',
      };
    }
